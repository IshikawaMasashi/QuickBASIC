    ...
    printf(...args: any[]) {
        ...
        for (i = 0; i < matches.length; i++) {
            ...
            if (matches[i].code == '%') { substitution = '%'; }
                ...
                matches[i].argument =
                    String(Math.abs(parseFloat(matches[i].argument)).toFixed(matches[i].precision ? <any>matches[i].precision : 6));
                substitution = convert(matches[i]);
            }
                ...
                matches[i].argument = matches[i].argument.substring(0, matches[i].precision ? <any>matches[i].precision : matches[i].argument.length);
                substitution = convert(matches[i], true);
            }
