...
export const SystemFunctions: IStringDictionary<ISystemFunction> = {
  ...
  "MID$": {
    ...
    "action": function (vm: VirtualMachine) {
      ...
      var start = <number>vm.stack.pop();
      var str = <string>vm.stack.pop();
  ...
  "SIN": {
    ...
    minArgs: 1,
    "action": function (vm: VirtualMachine) {
      let value = vm.stack.pop() as number;
  ...
  "EOF": {
    ...
    minArgs: 1,
    "action": function (vm: VirtualMachine) {
      let fileNumber = vm.stack.pop() as string;
      // Returns the square root of a numeric expression.
      // vm.stack.push(Math.sqrt(value));
