...
function printUsing(format: string, values: number[]) {
  let splitPositions: ([number, number])[] = [];
  let start = 0;
  let end = 0;
  let exponential = "";
  ...
  for (let splitPosition of splitPositions) {
...
export const SystemSubroutines: IStringDictionary<ISystemSubroutine> = {
  ...
  print_using: {
    action: function(vm: VirtualMachine) {
      // pop # args
      var argCount = vm.stack.pop() as number;
      ...
      let output = printUsing(formatString, values);
  ...
  LOCATE: {
    ...
    action: function(vm: VirtualMachine) {
      var col = (<ScalarVariable>vm.stack.pop()).value;
      var row = (<ScalarVariable>vm.stack.pop()).value;
  ...
  COLOR: {
    ...
    action: function(vm: VirtualMachine) {
      ...
      var fg = (<ScalarVariable>vm.stack.pop()).value;
  ...
  INPUT: {
    action: function(vm: VirtualMachine) {
      ...
      // user to numbers.
      var argCountOrFileNumber = vm.stack.pop();
